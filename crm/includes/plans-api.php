<?php
/**
 * Fetch and normalize Maz CRM plans from the portal public catalog API.
 */

if (!defined('CRM_PLANS_API_BASE')) {
 ...
 */
function crm_fetch_portal_plans($forceRefresh = false) {
    $cacheFile = __DIR__ . '/../cache/plans.json';
    $ttl = defined('CRM_PLANS_CACHE_TTL') ? (int) CRM_PLANS_CACHE_TTL : 600;

    if (!$forceRefresh && is_file($cacheFile)) {
        $cached = crm_read_plans_cache($cacheFile, $ttl);
        if ($cached !== null) {
            return [
                'plans' => $cached['plans'],
                'source' => 'cache',
    ...
    $live = crm_request_portal_plans();
    if ($live !== null && !empty($live)) {
        crm_write_plans_cache($cacheFile, $live);
        return [
            'plans' => $live,
            'source' => 'live',
        ];
    ...
    if (is_file($cacheFile)) {
        $stale = crm_read_plans_cache($cacheFile, PHP_INT_MAX);
 ...
 */
function crm_request_portal_plans() {
    $base = rtrim(CRM_PLANS_API_BASE, '/');
    $url = $base . '/plans?status=active&product_code=crm';
    $timeout = defined('CRM_PLANS_TIMEOUT') ? (int) CRM_PLANS_TIMEOUT : 5;

    $raw = crm_http_get($url, $timeout);
    if ($raw === null || $raw === '') {
        return null;
    }
    ...
    foreach ($json['data'] as $row) {
        ...
        $normalized = crm_normalize_portal_plan($row);
        if ($normalized !== null) {
            $plans[] = $normalized;
        }
    }
 ...
 */
function crm_http_get($url, $timeout = 5) {
    ...
    if (function_exists('curl_init')) {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_HTTPHEADER => [
                ...
                'User-Agent: MazCrmMarketing/1.0',
            ],
        ]);
        $body = curl_exec($ch);
    ...
    if ($curlBin !== null) {
        ...
        if ($tmpBody !== false && $tmpHdr !== false) {
            $cmd = escapeshellarg($curlBin)
                ...
                . ' -H ' . escapeshellarg('User-Agent: MazCrmMarketing/1.0')
 ...
 * @return array|null
 */
function crm_normalize_portal_plan(array $p) {
    ...
    $icon = $iconMap[$key] ?? ['blue', 'fa-filter'];
    ...
    return [
        ...
        'yearly' => $yearly,
        'currency' => $currency,
        'monthly_label' => $isQuote || $monthly <= 0 ? '' : crm_format_money($monthly, $currency),
        'yearly_label' => $isQuote || $yearly <= 0 ? '' : crm_format_money($yearly, $currency),
...
function crm_format_money($amount, $currency = 'INR') {
    $amount = (float) $amount;
    if (strtoupper($currency) === 'INR') {
        return '₹' . crm_format_indian_number($amount);
    }
    return strtoupper($currency) . ' ' . number_format($amount, $amount == floor($amount) ? 0 : 2);
}

function crm_format_indian_number($amount) {
    $amount = (float) $amount;
 ...
 */
function crm_normalize_fallback_plans(array $staticPlans) {
    $out = [];
    foreach ($staticPlans as $i => $plan) {
        ...
        $out[] = [
            ...
            'icon_fa' => (string) ($plan['icon_fa'] ?? 'fa-filter'),
 ...
 */
function crm_read_plans_cache($cacheFile, $ttl) {
...
function crm_write_plans_cache($cacheFile, array $plans) {
 ...
 */
function crm_resolve_pricing_plans(array $fallbackStatic = []) {
    $result = crm_fetch_portal_plans();
    ...
    $fallback = crm_normalize_fallback_plans($fallbackStatic);
    return [
        'plans' => $fallback,
        'source' => 'fallback',
        'live' => false,
