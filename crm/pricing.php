...
$resolved = crm_resolve_pricing_plans($crm_plans);
...
$heroCompare = $planNames
    ? ('Compare ' . (count($planNames) > 1
        ...
        : $planNames[0]) . '. Start free or talk to us for a plan that matches your sales team.')
    : 'Compare CRM plans for your team. Start free or book a demo.';

$pricing_faqs = [
  [
    'q' => 'Is pricing final on this page?',
    'a' => $plansLive
      ? 'Displayed amounts come from the live Maz CRM catalog. Taxes may apply. Contact us if you need a custom quote for larger teams.'
  ...
  [
    ...
    'a' => 'Yes. Use Start Free to begin in the product, or Book a Demo for a guided walkthrough of leads, pipeline, follow-ups, and reports.',
  ],
  [
    'q' => 'Do plans differ by reporting?',
    'a' => 'Core CRM includes lead, follow-up, and pipeline views. Advanced analytics, custom fields, and some automation may depend on your plan or add-ons.',
  ],
  [
...
<section class="crm-page-hero text-center">
  <div class="container">
    <nav class="crm-breadcrumb justify-content-center" aria-label="Breadcrumb">
      <a href="<?php echo htmlspecialchars(crm_url('index')); ?>">Home</a> / Pricing
    </nav>
    <div class="crm-eyebrow">Pricing</div>
    <h1>CRM software pricing that scales with your team</h1>
    <p class="mx-auto"><?php echo htmlspecialchars($heroCompare); ?></p>
    <div class="crm-billing-toggle mt-3" data-crm-billing>
...
<section class="crm-section pt-2">
  <div class="container">
    <div class="row g-3 g-lg-4 justify-content-center">
      <?php foreach ($plans as $plan):
        ...
        $primaryCta = $isQuote ? crm_url('contact') : CRM_SIGNUP_URL;
        $primaryLabel = $isQuote ? 'Book a Demo' : 'Start Free';
      ?>
      <div class="<?php echo htmlspecialchars($colClass); ?>">
        <div class="crm-plan-card <?php echo !empty($plan['popular']) ? 'is-popular' : ''; ?>">
          <?php if (!empty($plan['popular'])): ?>
            <div class="crm-popular-banner">MOST POPULAR</div>
          <?php endif; ?>
          <div class="crm-plan-icon <?php echo htmlspecialchars($plan['icon'] ?? 'blue'); ?>">
            <i class="fa-solid <?php echo htmlspecialchars($plan['icon_fa'] ?? 'fa-filter'); ?>" aria-hidden="true"></i>
          </div>
          ...
          <div class="d-grid gap-2">
            ...
            <?php if (!$isQuote): ?>
              <a href="<?php echo htmlspecialchars(crm_url('contact')); ?>" class="btn btn-link">Book a Demo</a>
            <?php else: ?>
              <a href="<?php echo htmlspecialchars(CRM_SIGNUP_URL); ?>" class="btn btn-link">Start Free</a>
            <?php endif; ?>
          </div>
        </div>
      </div>
...
<section class="crm-section">
  <div class="container" style="max-width:44rem;">
    ...
    <div class="accordion crm-faq" id="pricingFaq">
